...
```python
from lumina.rag import RagCapability
...
```
lumina_RAG_BACKEND=sqlite              # or "pgvector"
lumina_RAG_EMBEDDING_MODEL=voyageai:voyage-4
lumina_RAG_STORE_PATH=data/rag.db      # used when backend=sqlite
lumina_RAG_PGVECTOR_DSN=               # used when backend=pgvector
lumina_RAG_CHUNK_SIZE=512
lumina_RAG_CHUNK_OVERLAP=64
lumina_RAG_TOP_K=5
```
...
**`pgvector`** — External PostgreSQL with the `vector` extension. Point `lumina_RAG_PGVECTOR_DSN` at your Postgres instance.
...
lumina_RAG_EMBEDDING_MODEL=ollama:nomic-embed-text
OLLAMA_HOST=http://192.168.1.50:11434

# Any OpenAI-compatible embedding server
lumina_RAG_EMBEDDING_MODEL=openai:my-model
