...
SCRIPT_NAME="lumina-diagnostics"
DEFAULT_DAYS=1
...
log "=== Lumina-PDF Diagnostics ==="
log "This tool collects a diagnostic bundle for Lumina-PDF."
...
if [ "$MODE" = "1" ]; then
  ...
  if prompt_yes_no "Include /tmp/Lumina-PDF?" "n"; then INCLUDE_TEMP=true; fi
...
if [ -f /etc/lumina_version ]; then
  write_kv "lumina_version_file" "$(tr -d '\r\n' < /etc/lumina_version)" "$SUMMARY"
fi
...
if [ "$INCLUDE_TEMP" = true ]; then
  log "Collecting /tmp/Lumina-PDF..."
  copy_dir_filtered "/tmp/Lumina-PDF" "$DATA_DIR" 0
fi
...
write_tree "/tmp/Lumina-PDF" "${TREE_DIR}/tmp-Lumina-PDF.txt"
