...
export function useIndexedDBThumbnail(
  file: luminaFileStub | undefined | null,
): {
  ...
  const { updateluminaFileStub } = useFileManagement();
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (file.id && indexedDB && thumbnail) {
          try {
            ...
            updateluminaFileStub(file.id as FileId, {
    ...
    // stuck forever).
  }, [file, file?.thumbnailUrl, file?.id, indexedDB, updateluminaFileStub]);
