    ...
    case "$build_type" in
        full)
            ...
            if [ "$enable_security" == "true" ]; then
                ...
                service_name="Lumina-PDF-Security-Fat"
            else
                compose_file="${docker_compose_base}-fat${compose_suffix}"
                service_name="Lumina-PDF-fat"
            fi
            ;;
        ultra-lite)
            ...
            if [ "$enable_security" == "true" ]; then
                ...
                service_name="Lumina-PDF-backend-ultra-lite-security"
            else
                compose_file="${docker_compose_base}-ultra-lite${compose_suffix}"
                service_name="Lumina-PDF-Ultra-Lite"
            fi
            ;;
    esac
