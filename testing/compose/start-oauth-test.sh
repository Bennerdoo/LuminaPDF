...
echo -e "${BLUE}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Lumina PDF + Keycloak OAuth Test Environment  ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ "${SKIP_OAUTH_PREFLIGHT:-false}" != "true" ]; then
    if ! curl -sf --connect-timeout 2 --max-time 3 "http://${KEYCLOAK_HOST}:9080/realms/lumina-oauth" >/dev/null 2>&1; then
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if curl -sf http://localhost:9080/realms/lumina-oauth > /dev/null 2>&1; then
...
echo ""
echo -e "${YELLOW}▶ Starting Lumina PDF...${NC}"
docker-compose -f docker-compose-keycloak-oauth.yml up "${COMPOSE_UP_ARGS[@]}" Lumina-PDF-oauth

echo ""
echo -e "${YELLOW}▶ Waiting for Lumina PDF...${NC}"
WAITED=0
while [ $WAITED -lt $MAX_WAIT ]; do
    if curl -sf http://localhost:8080/api/v1/info/status 2>/dev/null | grep -q "UP"; then
        echo -e "${GREEN}✓ Lumina PDF is ready${NC}"
        break
    fi
    echo -n "."
    sleep 2
...
if [ $WAITED -ge $MAX_WAIT ]; then
    echo -e "${RED}✗ Lumina PDF failed to start${NC}"
    exit 1
fi
...
echo -e "   Lumina PDF:   ${GREEN}http://localhost:8080${NC}"
