package lumina.software.proprietary.security.database.repository;
...
public interface SessionRepository extends JpaRepository<SessionEntity, String> {
    ...
    @Query(
            "SELECT t.id as teamId, MAX(s.lastRequest) as lastActivity "
                    + "FROM lumina.software.proprietary.model.Team t "
                    + "LEFT JOIN t.users u "
    ...
    @Query(
            "SELECT u.username as username, MAX(s.lastRequest) as lastRequest "
                    + "FROM lumina.software.proprietary.security.model.User u "
                    + "LEFT JOIN SessionEntity s ON u.username = s.principalName "
