...
usage() {
    ...
    echo "  $0 --rerun \"Lumina-PDF-Regression Lumina-PDF-Security-Fat-with-login,Webpage-Accessibility-full\""
    exit 0
}
...
prepare_base_image() {
    if [ "${DOCKER_BASE_CHANGED:-false}" = "true" ]; then
        ...
        if docker build -f "$PROJECT_ROOT/docker/base/Dockerfile" \
            -t Lumina-PDF-base:local \
            "$PROJECT_ROOT/docker/base"; then
            echo "✓ Built base image locally: Lumina-PDF-base:local"
            BASE_IMAGE_ARG="--build-arg BASE_IMAGE=Lumina-PDF-base:local"
        else
...
main() {
    ...
    if should_run_test "Lumina-PDF-Ultra-Lite" || \
       should_run_test "Webpage-Accessibility-lite" || \
       should_run_test "Lumina-PDF-Ultra-Lite-Version-Check"; then
        ...
        if [ -n "${ACTIONS_RUNTIME_TOKEN}" ] && { [ -n "${ACTIONS_RESULTS_URL}" ] || [ -n "${ACTIONS_CACHE_URL}" ]; }; then
            DOCKER_CACHE_ARGS_ULTRA_LITE="--cache-from type=gha,scope=Lumina-PDF-ultra-lite --cache-to type=gha,mode=max,scope=Lumina-PDF-ultra-lite"
        else
    ...
    run_tests "Lumina-PDF-Ultra-Lite" "./docker/embedded/compose/docker-compose-latest-ultra-lite.yml"
    ...
    if should_run_test "Lumina-PDF-Ultra-Lite-Version-Check"; then
        start_test_timer "Lumina-PDF-Ultra-Lite-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "Lumina-PDF-Ultra-Lite" "http://localhost:8080"; then
            passed_tests+=("Lumina-PDF-Ultra-Lite-Version-Check")
            echo "Version verification passed for Lumina-PDF-Ultra-Lite"
        else
            failed_tests+=("Lumina-PDF-Ultra-Lite-Version-Check")
            capture_failure_logs "Lumina-PDF-Ultra-Lite-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for Lumina-PDF-Ultra-Lite"
        fi
        stop_test_timer "Lumina-PDF-Ultra-Lite-Version-Check"
    fi
    ...
    if should_run_test "Lumina-PDF-Security-Fat" || \
       should_run_test "Webpage-Accessibility-full" || \
       should_run_test "Lumina-PDF-Security-Fat-Version-Check" || \
       should_run_test "Lumina-PDF-Security-Fat-with-login" || \
       should_run_test "Lumina-PDF-Regression Lumina-PDF-Security-Fat-with-login" || \
       should_run_test "Lumina-PDF-Fat-Disable-Endpoints" || \
       should_run_test "Disabled-Endpoints" || \
       should_run_test "Lumina-PDF-Fat-Disable-Endpoints-Version-Check"; then
        ...
        if [ -n "${ACTIONS_RUNTIME_TOKEN}" ] && { [ -n "${ACTIONS_RESULTS_URL}" ] || [ -n "${ACTIONS_CACHE_URL}" ]; }; then
            DOCKER_CACHE_ARGS_FAT="--cache-from type=gha,scope=Lumina-PDF-fat --cache-to type=gha,mode=max,scope=Lumina-PDF-fat"
        else
    ...
    run_tests "Lumina-PDF-Security-Fat" "./docker/embedded/compose/docker-compose-latest-fat-security.yml"
    ...
    if should_run_test "Lumina-PDF-Security-Fat-Version-Check"; then
        start_test_timer "Lumina-PDF-Security-Fat-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "Lumina-PDF-Security-Fat" "http://localhost:8080"; then
            passed_tests+=("Lumina-PDF-Security-Fat-Version-Check")
            echo "Version verification passed for Lumina-PDF-Security-Fat"
        else
            failed_tests+=("Lumina-PDF-Security-Fat-Version-Check")
            capture_failure_logs "Lumina-PDF-Security-Fat-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for Lumina-PDF-Security-Fat"
        fi
        stop_test_timer "Lumina-PDF-Security-Fat-Version-Check"
    fi
    ...
    # ==================================================================
    run_tests "Lumina-PDF-Security-Fat-with-login" "./docker/embedded/compose/test_cicd.yml"

    # Only run behave tests if the container started successfully
    if [[ " ${passed_tests[*]} " =~ "Lumina-PDF-Security-Fat-with-login" ]]; then
        ...
        cd "testing/cucumber"
        start_test_timer "Lumina-PDF-Regression"
        ...
        if python -m behave \
            ...
            if compare_file_lists "$BEFORE_FILE" "$AFTER_FILE" "$DIFF_FILE" "$CONTAINER_NAME"; then
                ...
                passed_tests+=("Lumina-PDF-Regression $CONTAINER_NAME")
            else
                ...
                test_failure_logs["Lumina-PDF-Regression-Temp-Files"]="$tempfile_log"
                failed_tests+=("Lumina-PDF-Regression-Temp-Files")
            fi
            passed_tests+=("Lumina-PDF-Regression $CONTAINER_NAME")
        else
            gha_endgroup
            failed_tests+=("Lumina-PDF-Regression $CONTAINER_NAME")
            ...
            test_failure_logs["Lumina-PDF-Regression"]="$cucumber_log"
        ...
        stop_test_timer "Lumina-PDF-Regression"
    fi
    ...
    # ==================================================================
    run_tests "Lumina-PDF-Fat-Disable-Endpoints" "./docker/embedded/compose/docker-compose-latest-fat-endpoints-disabled.yml"
    ...
    if should_run_test "Lumina-PDF-Fat-Disable-Endpoints-Version-Check"; then
        start_test_timer "Lumina-PDF-Fat-Disable-Endpoints-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "Lumina-PDF-Fat-Disable-Endpoints" "http://localhost:8080"; then
            passed_tests+=("Lumina-PDF-Fat-Disable-Endpoints-Version-Check")
            echo "Version verification passed for Lumina-PDF-Fat-Disable-Endpoints"
        else
            failed_tests+=("Lumina-PDF-Fat-Disable-Endpoints-Version-Check")
            capture_failure_logs "Lumina-PDF-Fat-Disable-Endpoints-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for Lumina-PDF-Fat-Disable-Endpoints"
        fi
        stop_test_timer "Lumina-PDF-Fat-Disable-Endpoints-Version-Check"
    fi
