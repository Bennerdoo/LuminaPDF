...
import lumina.software.common.util.SpringContextHolder;
...
public class JobQueue implements SmartLifecycle {
    ...
    @Value("${lumina.job.queue.base-capacity:10}")
    private int baseQueueCapacity = 10;

    @Value("${lumina.job.queue.min-capacity:2}")
    private int minQueueCapacity = 2;

    @Value("${lumina.job.queue.check-interval-ms:1000}")
    private long queueCheckIntervalMs = 1000;

    @Value("${lumina.job.queue.max-wait-time-ms:600000}")
