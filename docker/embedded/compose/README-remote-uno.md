# Lumina-PDF with Remote UNO Servers

This docker-compose configuration demonstrates running Lumina-PDF with **separate UNO server containers** for LibreOffice document conversion, enabling horizontal scaling and better resource isolation.
...
# Watch main app
docker compose -f docker-compose-latest-security-remote-uno.yml logs -f Lumina-PDF
```
...
```
NAME                           STATUS
Lumina-PDF-Security-Remote-UNO   Up (healthy)
...
2. Add environment variables to Lumina-PDF service:
```yaml
...
PROCESS_EXECUTOR_SESSION_LIMIT_LIBRE_OFFICE_SESSION_LIMIT: "2"
# Creates 2 servers on 127.0.0.1:2003, 127.0.0.1:2005 inside container (Lumina-PDF's own servers)
```
