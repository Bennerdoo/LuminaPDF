...
public class GeneralUtils {
    ...
    public File convertMultipartFileToFile(MultipartFile multipartFile) throws IOException {
        String customTempDir = System.getenv("lumina_TEMPFILES_DIRECTORY");
        if (customTempDir == null || customTempDir.isEmpty()) {
            customTempDir = System.getProperty("lumina.tempfiles.directory");
        }
        ...
        if (customTempDir != null && !customTempDir.isEmpty()) {
            ...
            tempFile = Files.createTempFile(tempDir, "lumina-pdf-", null).toFile();
        } else {
            Path tempDir = Path.of(System.getProperty("java.io.tmpdir"), "lumina-pdf");
            if (!Files.exists(tempDir)) {
                Files.createDirectories(tempDir);
            }
            tempFile = Files.createTempFile(tempDir, "lumina-pdf-", null).toFile();
