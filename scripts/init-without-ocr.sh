#!/bin/bash
# This script initializes Lumina PDF without OCR features.
set -euo pipefail
  ...
  case "$JVM_PROFILE" in
    ...
    *)
      if [ -n "${_JVM_OPTS_BALANCED:-}" ]; then
        ...
        JAVA_BASE_OPTS="-XX:+ExitOnOutOfMemoryError -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/Lumina-PDF/heap_dumps -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=4m -XX:G1PeriodicGCInterval=60000 -XX:+UseStringDeduplication -XX:+UseCompactObjectHeaders -XX:+ExplicitGCInvokesConcurrent -Dspring.threads.virtual.enabled=true"
      fi
      ;;
  esac
...
log "Running Lumina PDF with DISABLE_ADDITIONAL_FEATURES=${DISABLE_ADDITIONAL_FEATURES:-} and VERSION_TAG=${VERSION_TAG:-<unset>}"
...
if [[ "${INSTALL_BOOK_AND_ADVANCED_HTML_OPS:-false}" == "true" && "${FAT_DOCKER:-true}" != "true" ]]; then
  log "issue with calibre in current version, feature currently disabled on Lumina-PDF"
fi
...
mkdir -p /tmp/Lumina-PDF /tmp/Lumina-PDF/heap_dumps /logs /configs /configs/heap_dumps /configs/cache /customFiles /pipeline || true
CHOWN_PATHS=("$HOME" "/logs" "/scripts" "/configs" "/customFiles" "/pipeline" "/tmp/Lumina-PDF" "/app.jar")
...
# ---------- Java ----------
# Start Lumina PDF Java application immediately (parallel with unoserver startup).
log "Starting Lumina PDF"
JAVA_CMD=(
  java
  -Dfile.encoding=UTF-8
  -Djava.io.tmpdir=/tmp/Lumina-PDF
)
...
exit_code=$?
case "$exit_code" in
  0)   log "Lumina PDF exited normally." ;;
  137) log "Lumina PDF was OOM-killed (exit 137). Check container memory limits." ;;
  143) log "Lumina PDF terminated by SIGTERM (normal orchestrator shutdown)." ;;
  *)   log "Lumina PDF exited with code ${exit_code}." ;;
esac
