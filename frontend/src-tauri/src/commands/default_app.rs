...
/// Check if Lumina PDF is the default PDF handler
#[tauri::command]
...
/// Attempt to set/prompt for Lumina PDF as default PDF handler
#[tauri::command]
...
fn check_default_linux() -> Result<bool, String> {
    ...
    let is_default = handler.trim() == "Lumina-PDF.desktop";
    Ok(is_default)
}
...
fn set_default_linux() -> Result<String, String> {
    ...
    let result = Command::new("xdg-mime")
        .args(["default", "Lumina-PDF.desktop", "application/pdf"])
        .output()
