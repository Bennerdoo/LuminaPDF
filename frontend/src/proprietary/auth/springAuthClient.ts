...
const OAUTH_REDIRECT_COOKIE = "lumina_redirect_path";
const OAUTH_REDIRECT_COOKIE_MAX_AGE = 60 * 5; // 5 minutes
const DEFAULT_REDIRECT_PATH = `${BASE_PATH || ""}/auth/callback`;

export const POST_LOGIN_REDIRECT_STORAGE_KEY = "lumina_post_login_path";
...
class SpringAuthClient {
  ...
  async getSession(): Promise<{
    ...
    try {
      ...
      let token = localStorage.getItem("lumina_jwt");
      ...
      if (await isDesktopSaaSAuthMode()) {
        ...
        if (tokenExpiry.expiresIn <= this.DESKTOP_SAAS_REFRESH_EARLY_SECONDS) {
          ...
          const refreshedToken = localStorage.getItem("lumina_jwt");
          if (!refreshedToken) {
            localStorage.removeItem("lumina_jwt");
            return { data: { session: null }, error: null };
          }
   ...
   *
   * @param params.provider - Full auth path from backend (e.g., '/oauth2/authorization/google', '/saml2/authenticate/lumina')
    ...
    try {
      if (typeof window !== "undefined") {
        window.sessionStorage.setItem(
          "lumina_sso_auto_login_logged_out",
          "1",
        );
      }
