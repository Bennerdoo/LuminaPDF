 ...
 * 1. window.LUMINA_PDF_API_BASE_URL (runtime override - fixes hardcoded localhost issues)
...
export function getApiBaseUrl(): string {
  if (!isTauri()) {
    ...
    if (typeof window !== "undefined" && window.LUMINA_PDF_API_BASE_URL) {
      return window.LUMINA_PDF_API_BASE_URL;
    }
